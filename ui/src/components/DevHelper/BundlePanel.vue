  ...
  <div>
    <b-field>
      <template #label>
        ...
        <b-button v-if="showSubmitBundle && bundle" size="is-small" class="is-pulled-right" @click="submit()">
            ...
            <li v-for="(sol, i) in solution_results" :key="i">
              ...
              <ul v-if="sol.args.length > 0" class="args_list ellipsis-item">
                <li v-for="(arg, i) in sol.args" :key="i">
                  ...
                  >
                  <span v-else-if="sol.code == 66 || sol.code == 67" class="has-text-weight-bold" title="Bytes32"
                    >{{ getMessageModeByCode(sol.args[0], sol.code).arguments[i - 2].name }}:</span
                  >
                  {{ getArgMsg(arg) }}
...
import puzzle, { ConditionArgs, ConditionEntity } from "../../../../lib-chia/services/crypto/puzzle";
import { beautifyLisp } from "../../../../lib-chia/services/coin/lisp";
import {
  conditionDict,
  ConditionInfo,
  prefix0x,
  ...
  getArgMsg,
  getFirstLevelArg,
} from "../../../../lib-chia/services/coin/condition";
...
import { gerMermaidDiagramDefinition } from "@/services/dev/diagramRender";
...
export default class BundlePanel extends Vue {
  ...
  public async renderMermaid(): Promise<void> {
    ...
    const graphDefinition = gerMermaidDiagramDefinition(
      this.aggSigMessages,
      this.coinMods,
      this.puzzleAnnoCreates,
      this.puzzleAnnoAsserted,
