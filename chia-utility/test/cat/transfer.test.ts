...
test("Cat Transfer", async () => {
  ...
  const plan = await transfer.generateSpendPlan(
    { CAT: [coin] },
    [{ symbol: "CAT", address: tgt_hex, amount: 300n, memos: [tgt_hex] }],
    change_hex,
    0n,
    net.symbol
  );
...
test("Multi Cat Coin Transfer", async () => {
  ...
  const plan = await transfer.generateSpendPlan(
    availcoins,
    [{ symbol: "CAT", address: tgt_hex, amount: 1900n, memos: [tgt_hex] }],
    change_hex,
    0n,
    net.symbol
  );
...
test("Multi Cat Coin Transfer 2", async () => {
  ...
  const puzzles = await puzzle.getCatPuzzleDetails(utility.fromHexString(sk_hex), assetId, xchPrefix(), 0, 8, "cat_v2");
  expect(puzzles).toMatchSnapshot("puzzles");
  const plan = transfer.generateSpendPlan(
    availcoins,
    [{ symbol: "CAT", address: tgt_hex, amount: 60000n, memos: [tgt_hex] }],
    change_hex,
    0n,
    net.symbol
  );
