...
export async function initCnsCoin(
  ...
  if (oanalysis == null) assert.fail("null analysis");
  if (!("cnsExpiry" in oanalysis)) assert.fail("none-cns analysis");
...
export async function testUpdateCns(
  ...
  if (finalAnalysis == null) assert.fail("null finalAnalysis");
...
describe("Update CNS from empty", () => {
  ...
  test("1: to one address", async () => {
    ...
    if (!initData) assert.fail("not init");
