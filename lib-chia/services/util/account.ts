...
export async function getAccountAddressDetails(
  ...
  if (account.type == "PublicKey") throw new Error("Public Key accound cannot provide puzzle detail");

  account.addressPuzzles = await receive.getAssetsRequestDetail(
...
export async function getAccountPuzzleObservers(
  ...
  if (account.type == "PublicKey") {
    if (!account.key.publicKey) throw new Error("public key cannot empty for pk account to get account addresses.");
    account.observePuzzles = await receive.getAssetsRequestObserver(
