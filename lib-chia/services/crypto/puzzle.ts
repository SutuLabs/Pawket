...
class PuzzleMaker {
  ...
  public async getCatPuzzleDetails(
    ...
    return await this.getPuzzleDetailsInner(
      privateKey,
  ...
  public async getPuzzleObservers(
  ...
  public async getCatPuzzleObservers(
    ...
    return await this.getPuzzleObserversInner(
      pubKey,
      async (spk) => this.getCatPuzzle(spk, assetId, catModName),
      startIndex,
      endIndex,
  ...
  public async getCatPuzzleHashes(
    privateKey: Uint8Array,
    assetId: string,
    prefix: string,
    startIndex = 0,
    ...
    try {
      ...
      const conds = prog.map<ConditionEntity>((cond: TToJavascript) => ({
        code: !Array.isArray(cond)
          ? cond.raw()[0]
          : !Array.isArray(cond[0])
          ? cond[0].raw()[0]
          : !Array.isArray(cond[0][0])
          ? cond[0][0].raw()[0]
          : -1,
        args: Array.isArray(cond)
          ? cond.slice(1).map((_: TToJavascript) =>
              Array.isArray(_)
                ? Array.from(_).map((c: TToJavascript) =>
                    Array.isArray(c) // utilize Array.from to convert Tuple to Array, otherwise `object is not extensible` error thrown
                      ? Array.isArray(c[0])
                        ? undefined
                        : c[0].raw()
                      : c.raw()
