...
test('Make Offer 1', async () => {
  ...
  const bundle = await generateOffer(offplan, reqs, tokenPuzzles, noNeedGetProof, xchSymbol(), chainId(), nonce, "cat");
  const encoded = await encodeOffer(bundle);
...
test('Make Offer 2', async () => {
  ...
  const bundle = await generateOffer(offplan, reqs, tokenPuzzles, localPuzzleApiCall, xchSymbol(), chainId(), nonce, "cat");
  const encoded = await encodeOffer(bundle);
...
test('Take Offer Xch For CAT', async () => {
  ...
  const takerBundle = await generateOffer(offplan, revSummary.requested, tokenPuzzles, localPuzzleApiCall, xchSymbol(), chainId(), nonce, "cat");
